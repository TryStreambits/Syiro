...
namespace syiro.toast {
	...
	export function New(properties: ToastPropertiesObject): ComponentObject {
		...
		let componentElement: Element = syiro.utilities.ElementCreator("div", { "data-syiro-component-id": componentId, "data-syiro-component": "toast" }); // Generate the Toast container

		// #region Title Generation / Properties Redirect

		if (typeof properties.title == "string") { // If a title is defined
			if (typeof properties.message == "string") { // If a message is defined (meaning don't do a properties change
		...
		if (typeof properties.title == "string") { // If both a title and message are provided (only check title since if title is provided but no message, it is automatically changed to message and therefore title = undefined
	...
	export function Toggle(component: ComponentObject, action?: string) {
		...
		if (componentElement !== null) { // If the componentElement exists
			...
			if (typeof action !== "string") { // If an action wasn't provided or it wasn't a string ("show" / "hide")
				if ((syiro.device.width > 1024) && (currentAnimation == "slide")) { // If this is non-mobile and animation was slide
					showAnimation = false; // Slide Out the Normal Toast
				} else if ((syiro.device.width <= 1024) && ((currentAnimation == "fade-in") || (currentAnimation == "slide"))) { // If this is mobile  and we did a Fade In or Slide (a Slide would happen if we had a large document width which "shrunk" down)
			...
			if (syiro.device.width > 1024) { // If this is non-mobile
			...
			} else if (syiro.device.width <= 1024) { // If this is mobile
				if (showAnimation) { // If we are showing the Toast	
